...
bool Cube::intersect(const Ray &ray, glm::vec3 &point, glm::vec3 &normal) {
	...
	if (tN > tF || tF < 0.0 || isnan(tF)) return false;

	point = rTemp.evalPoint(tN);
	//cout << "[" << imageX << ", " << imageY << "] = Point: " << point << endl << "     TMin: " << rdd << ", TMax: " << roo << endl;
	...
	normal = -glm::sign(rdd) * glm::step(glm::vec3(t1.y, t1.z, t1.x), t1)
		* glm::step(glm::vec3(t1.z, t1.x, t1.y), t1);
	normal = (M * glm::vec4(normal.x, normal.y, normal.z, 0.0));
	//cout << "     N: " << n << " /  K: " << invdir << " /  T1: " << t1 << "  /  T2: " << t2 << "  /  TN: " << tN << "  /  TF: " << tF << endl;
	return true;
}
